...
class VariablesTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        with pncpy.File(self.file_path,'w', format = "64BIT_OFFSET") as f:
            ...
            v.setncatts({'foo': np.int32(1)})
            v.setncatts(OrderedDict(bar=np.int32(2)))
            v.stratt_tmp = STRATT
    ...
    def test_unsupported(self):
        with pncpy.File(self.file_path, 'r+') as f:
            ...
            #should not be able to set variable with types not supported by cdf2
            try:
                v.longatt = LONGATT
            except TypeError:
                pass
    ...
    def test_fill_val(self):
        with pncpy.File(self.file_path, 'r+') as f:
            v = f.variables[VAR_NAME]
            #should not be able to set _FillValue after var creation
            try:
                v._FillValue(-999.)
