...
class VariablesTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        dx = f.defineDim('x',xdim)
        dy = f.defineDim('y',ydim)

        # define 20 netCDF variables
        for i in range(num_reqs * 2):
            v = f.defineVar(f'data{i}', pncpy.NC_FLOAT, (dx, dy))
        # initialize variable values
        f.enddef()
        for i in range(num_reqs * 2):
    ...
    def test_cdf5(self):
        """testing variable iget varn method for CDF-5 file format"""
        # test iget_varn and collective i/o wait_all
        for i in range(num_reqs * 2):
            assert_array_equal(v_datas[i], dataref)

    def test_cdf2(self):
        """testing variable iget varn method for CDF-2 file format"""
        # test iget_varn and collective i/o wait_all
        for i in range(num_reqs * 2):
            assert_array_equal(v_datas[i], dataref)

    def test_cdf1(self):
        """testing variable iget varn method for CDF-1 file format"""
        # test iget_varn and collective i/o wait_all
        for i in range(num_reqs * 2):
            assert_array_equal(v_datas[i], dataref)
