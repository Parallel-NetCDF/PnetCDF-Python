...
class FileTestCase(unittest.TestCase):
    ...
    def runTest(self):
        ...
        # TEST MODE "w"
        with pnetcdfpy.File(filename=self.file_path, mode='w', format=self.file_format, Comm=comm) as f:
        ...
        # TEST MODE "r"
        with pnetcdfpy.File(filename=self.file_path, mode='r') as f:
        # Try writing to file by defining a dimension
            try:
                f.redef()
                f.def_dim('x',xdim)
        ...
        # TEST MODE "a"
        with pnetcdfpy.File(filename=self.file_path, mode='a') as f:
            f.redef()
            # Try writing to file by defining a dimension
            f.def_dim('z',zdim)
            # Check if both 'x' and 'y' dimensions existed in file
        ...
        try:
            f = pnetcdfpy.File(filename=self.file_path, mode='x')
        except OSError:
            pass
        else:
