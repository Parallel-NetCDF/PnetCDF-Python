...
class FileTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # create netCDF file
        f = pnetcdfpy.File(filename=self.file_path, mode = 'w', format=self._file_format, \
        ...
        v1_u = f.def_var('data1u', pnetcdfpy.NC_INT, (dim_xu, dim_y, dim_z))
        v2_u = f.def_var('data2u', pnetcdfpy.NC_INT, (dim_xu, dim_y, dim_z))
        v1 = f.def_var('data1', pnetcdfpy.NC_INT, (dim_x, dim_y, dim_z))
        v2 = f.def_var('data2', pnetcdfpy.NC_INT, (dim_x, dim_y, dim_z))
    ...
    def runTest(self):
        ...
        if self._file_format == "64BIT_DATA":
            self.assertEqual(self.version, pnetcdfpy.NC_64BIT_DATA)
        elif self._file_format == "64BIT_OFFSET":
            self.assertEqual(self.version, pnetcdfpy.NC_64BIT_OFFSET)
        elif self._file_format == "CLASSIC":
            self.assertEqual(self.version, pnetcdfpy.NC_CLASSIC_MODEL)
