...
def main():
    ...
    fix_var = f.def_var("fix_var", pnetcdfpy.NC_INT, (dim_y, dim_x))
    rec_var =  f.def_var("rec_var", pnetcdfpy.NC_INT, (dim_xu, dim_x))
    # set the fill mode to NC_FILL for the entire file
    old_fillmode = f.set_fill(pnetcdfpy.NC_FILL)
    if verbose:
        if old_fillmode == pnetcdfpy.NC_FILL:
    ...
    f.set_fill(pnetcdfpy.NC_NOFILL)
    ...
    assert(no_fill == 0)
    assert(fill_value == pnetcdfpy.NC_FILL_INT)
